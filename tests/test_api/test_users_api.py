...
#     # Try fetching a non-existent user → should hit the 404 branch
#     random_id = uuid4()
#     headers = {"Authorization": f"Bearer {admin_token}"}
...
# @pytest.mark.asyncio
# async def test_upgrade_to_pro_insufficient_privileges(
#     async_client: AsyncClient,
#     db_session: AsyncSession,
#     user_token: str
# ):
#     """
#     An Authenticated (non-Admin/Manager) user should get 403.
#     Covers the first `if current_user.role not in ...` branch.
#     """
#     # Seed a target user in the DB
#     target = User(
#         id=uuid4(),
#         nickname=generate_nickname(),
#         first_name="Target",
...
#     headers = {"Authorization": f"Bearer {user_token}"}
#     resp = await async_client.post(f"/{target.id}/upgrade-pro", headers=headers)
#     assert resp.status_code == 403
#     # assert resp.json() == {"detail": "Insufficient privileges"}


# @pytest.mark.asyncio
# async def test_upgrade_to_pro_user_not_found(
#     async_client: AsyncClient,
#     db_session: AsyncSession,
#     admin_token: str
# ):
#     """
#     An Admin with a non-existent user_id should get 404.
#     Covers the `if not user: raise HTTPException(404)` branch.
#     """
#     random_id = uuid4()
#     headers = {"Authorization": f"Bearer {admin_token}"}
#     resp = await async_client.post(f"/{random_id}/upgrade-pro", headers=headers)
#     assert resp.status_code == 404
#     assert resp.json() == {"detail": "User not found"}
...
# ):
#     """
#     A Manager upgrading an existing user should succeed.
#     Covers the successful path: setting is_professional and timestamp.
#     """
#     # Seed a user with is_professional=False
#     target = User(
#         id=uuid4(),
#         nickname=generate_nickname(),
#         first_name="Target",
...
#     resp = await async_client.post(f"/{target.id}/upgrade-pro", headers=headers)
#     assert resp.status_code == 200
#     assert resp.json() == {"message": "Upgraded to professional status"}

#     # Refresh from DB and verify changes
#     refreshed = await db_session.get(User, target.id)
#     assert refreshed.is_professional is True
#     assert isinstance(refreshed.professional_status_updated_at, datetime)
